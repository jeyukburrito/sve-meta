  ...
  return (
    <article className="rounded-3xl border border-line bg-surface p-5 shadow-sm">
      ...
      <div className="relative mt-5 ml-3">
        ...
        <div className="space-y-0">
          {group.matches.map((match, idx) => (
            <div key={match.id} className="relative flex gap-4 pb-4 last:pb-0">
              {/* 도트 */}
              ...
              <div className="flex-1 rounded-2xl border border-line px-4 py-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold text-muted">R{idx + 1}</span>
      ...
      <div className="mt-3 ml-3 flex gap-4">
        ...
        <Link
          href={`/matches/new?event=${group.eventCategory}&date=${group.date.toISOString().slice(0, 10)}&deckId=${group.firstDeckId}&gameId=${group.firstGameId}&round=${group.matches.length + 1}`}
          className="text-sm font-medium text-accent hover:underline"
        >
          + 라운드 {group.matches.length + 1} 추가
        </Link>
      </div>
    </article>
  );
